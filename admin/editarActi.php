<?php
	...
	if ($_FILES["imagen_nueva"]["error"] > 0){
		...
		//Registra el usuario
		$query_new = mysqli_query($mysqli, "UPDATE actividades SET titulo='".$titulo."',info='".$info."' WHERE id='".$id."'");
		...
		if (in_array($_FILES['imagen_nueva']['type'], $permitidos) && $_FILES['imagen_nueva']['size'] <= $limite_kb * 1024){
			...
			$ruta = "../img/activ/" . $_FILES['imagen_nueva']['name'];
			$ruta_original = "../img/activ/" . $img_actual;
			...
			if (!file_exists($ruta)){
				...
				if ($resultado){
					...
					imagejpeg($tmp,"../img/activ/".$nombreImg,$calidad);


					//Registra el usuario
					$query_new = mysqli_query($mysqli, "UPDATE actividades SET titulo='".$titulo."',info='".$info."' ,img_act='".$nombreImg."' WHERE id='".$id."'");
